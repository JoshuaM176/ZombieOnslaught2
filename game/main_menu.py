...
class MainMenu(ScreenPage, ButtonContainer):
    ...
    def update(self):
        ...
        for button in self.buttons:
            button.update(
                deleting_profile=self.deleting_profile, selected_profile=self.profile
            )
        self.get_input()
        if self.creating_profile:
            text(
                self.screen,
                f"|{self.typed_input}",
                self.typed_input_size,
                self.screen.get_width() / 2,
                self.screen.get_height() * 0.65,
                align="CENTER",
            )
        return self.go2
...
class SelectSaveButton(Button):
    ...
    def update(self, deleting_profile, selected_profile, **kwargs):
        ...
        for i in range(
            self.scroll_index,
            min(self.scroll_index + round((self.height - 50) / 60), len(save_names)),
        ):
            if deleting_profile and i == selected_profile:
                text(
                    ...
                    60,
                    self.screen.get_width() / 2,
                    self.y + 50 + (i - self.scroll_index) * 60,
                    color=(100, 0, 0),
                    align="CENTER",
                )
            else:
