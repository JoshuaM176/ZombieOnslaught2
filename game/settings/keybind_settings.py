...
class SetKeyBindsButton(Button):
    ...
    def update_key_map_text(self):
        self.key_name_map = {1073742049: "SHIFT", 32: "_", 0: ""}
        self.action_text = []
        self.key1_text = []
        self.key2_text = []
        for action in self.actions:
            ...
            key1 = (
                self.key_name_map.get(values[0])
                if self.key_name_map.get(values[0]) is not None
                else f"Mouse{values[0]}"
                if values[0] < 10
                else chr(values[0])
            )
            key2 = ""
            if len(values) > 1:
                key2 = (
                    self.key_name_map.get(values[1])
                    if self.key_name_map.get(values[1]) is not None
                    else f"Mouse{values[1]}"
                    if values[1] < 10
                    else chr(values[1])
                )
        ...
        if self.action_selected:
            if self.actions.index(self.action_selected) - self.scroll_index < 0:
                ...
                pg.draw.rect(
                    ...
                    (100, 100, 0),
                    (
                        self.x + 500 + 250 * self.key_selected,
                        self.y + (self.actions.index(self.action_selected) - self.scroll_index) * 60 + 20,
                        250,
                        60,
                    ),
                )
