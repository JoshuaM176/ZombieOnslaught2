...
class WeaponRegistry():
    def __init__(self):
        ...
        for key, value in resources.items():
            value["weapon"]["sprites"] = convert_files_to_sprites(value["weapon"]["sprites"])
            weapon = {"name": key}
...
class EquippedWeaponRegistry():
    ...
    def equip(self, weapon: dict, cat: str):
        self.equipped = cat
        self.weapons[cat] = Weapon(**weapon)
    ...
    def set_next(self):
        if self.equipped_list.index(self.equipped) < len(self.equipped_list)-1:
            self.equipped = self.equipped_list[self.equipped_list.index(self.equipped)+1]
